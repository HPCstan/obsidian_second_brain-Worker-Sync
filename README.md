...
### 6. 🌐 語系與終端適配再進化：簡體中文支援與行動版網頁 (m.youtube.com) 突破
在廣泛的終端實測與語系適配中，我們進一步賦予了前端擴充套件「無死角」的極限解析力：

* **簡體中文介面與字幕全面兼容 (Simplified Chinese Support)**
  當影片僅提供簡體字幕，或使用者的 YouTube 介面為簡體中文時，YouTube 官方的 DOM 結構與按鈕文字會從「顯示轉錄稿」轉變為「显示转录稿」、「打开转录稿」等。
  最新版的擴充套件已完全注入了兩岸三地與多語系的正規表達式雷達（`/显示转录稿|打开转录稿|zh-Hans|zh-CN/`）。無論影片是純簡體字幕，或是介面語系差異，套件皆能如繁體環境般，一秒精準模擬點擊並無縫匯出完美筆記。

* **平板與行動版網頁的「降維打擊」與破局 (m.youtube.com)**
  在 Android 平板或手機使用 Kiwi Browser 開啟 YouTube 時，系統預設會載入行動版網頁 (`m.youtube.com`)。
  * **行動版 UI 的先天限制**：Google 刻意在行動版網頁移除了「顯示轉錄稿」按鈕，導致我們的「第一優先：DOM 直接提取 (`extractFromDOM`)」戰略無用武之地，進而被迫啟動會被 YouTube 阻擋的後端 API 下載，最終報錯 `(完全空白)`。
  * **一鍵突破法 (Desktop Site)**：本專案的終極解法極為純粹 —— 您只需在 Kiwi Browser 的選單中勾選 **「切換為電腦版網站 (Desktop site)」**！讓網址轉回 `www.youtube.com`，電腦版的 DOM 面板瞬間浮現，套件便能在此完美施展 100% 的擷取神技，一擊通關。
  * *(套件現已內建偵測機制，若誤觸 `m.youtube.com`，將會溫馨彈出提示引導您切換至電腦版。)*

* **新版 YouTube 混合式排版 (View Models) 終極破解法：純文字榨取 (innerText Parsing)**
  YouTube 於最新版的「影片相關資訊 (Engagement Panels)」介面中，開始逐步廢棄傳統的 `<ytd-transcript-segment-renderer>` 標籤，改用無固定特徵的 `yt-view-model` 框架將「影片章節」與「字幕」揉合顯示。
  * **1.5 階純文字防禦機制**：為了徹底免疫 HTML 標籤改版造成的「DOM 提取失敗」，本套件導入了極為霸道且絕對有效的「純文字暴力解析法」。
  * **無視 DOM 的肉眼擷取**：只要字幕面板在畫面上展開，套件將直接讀取瀏覽器渲染出的純文字 (`innerText`)。透過高階正規表達式掃描，自動濾除「章節標題」、「搜尋轉錄稿」等系統雜訊，精準捕捉所有 `0:24` 格式的時間戳與對白。這種作法確保了只要使用者的「肉眼」能看到字幕，套件就 100% 能無損抓下，達成了抗版面變動的最強韌性！
